...
The list above is basic, and there are many more complex functions like ``grep`` that can help you parse files. These functions are often significantly faster when run on then command line than in Python or R, so it is useful to get familiar with them. If you don't know how to do a certain function, google it, and you will find many useful hits on StackOverflow and other websites. 
